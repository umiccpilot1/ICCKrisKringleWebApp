...
async function testLinkPreviewAPI() {
  try {
    ...
    if (response.data.image) {
      console.log('\n✅ IMAGE FOUND:', response.data.image);
...
async function testOurEndpoint() {
  ...
  try {
    const response = await axios.get('http://localhost:3000/api/preview/link-preview', {
      params: { url: testUrl },
      timeout: 15000
    });
    ...
    if (response.data.image) {
      console.log('\n✅✅✅ SUCCESS! Our endpoint returns image!');
    } else {
      console.log('\n❌ Our endpoint returns no image');
    }
